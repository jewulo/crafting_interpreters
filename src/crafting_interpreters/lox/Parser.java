...
public class Parser {
    ...
    private Stmt declaration() {
        try {
            ...
            if (match(FUN)) return function("function");    // "fun" is already consumed
    ...
    private Stmt classDeclaration() {
        ...
        while (!check(RIGHT_BRACE) && !isAtEnd()) {
            methods.add(function("method"));    // consume method name and body using function(). It expects func_name() {body}, in this case method_name() {body}
        }
        // we have consumed all the methods names and bodies
    ...
    private Stmt.Function function(String kind) {
        ...
        List<Stmt> body = block(); // Note: parsing in block() already presumes that '{' is matched
    ...
    private Expr finishCall(Expr callee) {
        ...
        if (!check(RIGHT_PAREN)) {  // check to see if arguments are passed?
            do {
                if (arguments.size() >= 255) {
                    error(peek(), "Can't have more than 255 arguments."); // page 154. No Exception, just an error
                }
